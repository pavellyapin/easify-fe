...
exports.financialPlanOnCreate = functions.firestore
  .document("financialPlans/{planId}")
  .onCreate(async (snap) => {
    const planData = snap.data().financialPlan;
    const tags = planData.tags || [];
    const category = planData.category || "uncategorized"; // Default to 'uncategorized' if no category provided
    ...
    try {
      ...
      console.log(
        "Financial tags and category counts updated and sorted successfully.",
      );
    } catch (error) {
      console.error(
        "Error updating financial tags and category counts:",
        error,
      );
    }
  });
